  ...
  <script>
	function mInit() {
		if ("<%=session.getAttribute("vId") %>" != "<%=rv_writer %>") {
			alert('해당 리뷰는 작성자만 수정 및 삭제 가능합니다 !');
			location.href="<%=vURL%>";
			return;
		}
	}
...
<div id="wrap">
  ...
  <center>
    <div class="notice">
    <div class="bg_opac">
    <form action="<%=request.getContextPath() %>/Review_Upd_Proc.do" method="post" name="frmRV">
    ...
    <input type="hidden" name="rv_date" value="<%=rv_date %>">
      ...
      <table>
        <tr>
          <th colspan="2" class="title" style='height: 5%;'><h3>Review Update</h3></th>
        </tr>
